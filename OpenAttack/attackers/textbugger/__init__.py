...
class TextBuggerAttacker(ClassificationAttacker):
    ...
    def attack(self, victim: Classifier, sentence, goal: ClassifierGoal):
        ...
        for word_idx in ranked_words:
            ...
            bug = self.bug_sub_C(word)  # only use sub_C
    ...
    def get_word_importances(self, sentence_tokens, clsf, goal: ClassifierGoal):
        ...
        word_losses = [
            k
            for k, _ in sorted(
                word_losses.items(), key=lambda item: item[1], reverse=goal.targeted
            )
        ]
        return word_losses

    def get_w_word_importances(
        self, sentence_tokens, clsf, goal: ClassifierGoal
    ):  # white
        # get y_predict
        y_orig = goal.target
        ...
        if grad.shape[0] != len(sentence_tokens):
            raise RuntimeError(
                "Sent %d != Gradient %d" % (len(sentence_tokens), grad.shape[0])
            )
    ...
    # def generateBugs(
    #     self, word, glove_vectors, sub_w_enabled=False, typo_enabled=False
    # ):
    #     bugs = {
    #         "insert": word,
    #         "delete": word,
